...
[anaconda-anon-usage](https://crates.io/crates/anaconda-anon-usage).
It produces token strings identical to the Python package across all
token types, verified by automated parity tests. This enables
Rust-based package managers to provide the same telemetry if they wish.
See [`rust/README.md`](rust/README.md) for library usage, build
...
```
conda/26.1.1 requests/2.32.5 CPython/3.12.12 Darwin/25.2.0 OSX/26.2
solver/libmamba conda-libmamba-solver/25.11.0 libmambapy/2.3.2
aau/0.7.6 c/. s/. e/. a/. o/. m/.
```

The tokens preceding `aau/` constitute the standard user-agent string
that conda *normally* sends with HTTP requests, and are similar
to what a standard web browser such as Chrome, Safari, or Edge
send every time a page is requested. The `aau/` token and everything
after it are generated by the `anaconda-anon-usage` package. See the
[Token reference](#token-reference) section below for a complete
description of each token type.

Note that `conda info` redacts the token values for privacy, so
that your personal tokens are not inadvertently shared when you
include `conda info` output in bug reports or feature requests.
The package includes a CLI tool that shows the full, unredacted
token string as it is actually sent to the server:
...
```
aau/0.7.6 c/sgzzP8ytS_aywmqkDTJ69g s/3ItHH93LRUmCoJZkMOiD3g e/hCmim1vFSbinlm4waR6dZw
```

Use `--detail` to see where each token is coming from:
...
For convenience, the CLI includes an option to generate a random token
ready for use as a system token:
```
